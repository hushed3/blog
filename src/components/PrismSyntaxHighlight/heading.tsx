...
import { useStyles } from './style/heading'
...
const heading =
  ...
  ({ children }: HeadingProps) => {
    ...
    if (level === 4) {
      ...
      return (
        <Title level={4} id={`#${children}`} className={styles.heading}>
          <a id={`${children}`} className={styles.anchor} onClick={(e) => handleClick(e, children)}>
    ...
    return (
      <Title level={level} id={children as string}>
        {children}
      </Title>
    )
  }
